...
class ServiciosApiController extends ControllerBase
{
    ...
    public function notificar($nid, Request $request)
    {
        ...
        $estado = (string) ($this->getFieldValue($serv, 'field_estado') ?? '');
        if (in_array($estado, ['notificado', 'completado', 'finalizado'], true)) {
            return new JsonResponse([
        ...
        // ✅ Enviar correo y obtener resultado detallado
        $notify_result = $this->sendProximoServicioEmail($serv);

        // ❌ Si falla, NO cambiamos estado/fecha
        ...
        // ✅ Si éxito: estado + fecha_notificado
        $hoy = date('Y-m-d');
    ...
    public function informe($nid, Request $request)
    {
        ...
        return new JsonResponse(['message' => 'Informe asociado y servicio finalizado', 'id' => $serv->id()]);
    }
    ...
    private function sendNotificacionCliente(Node $serv): bool
    {
        // (Opcional) respetar flag del servicio
        ...
        // Respetar flag en cliente (si existe)
        if ($cliente && $cliente->hasField('field_enviar_notificaciones') && !$cliente->get('field_enviar_notificaciones')->isEmpty()) {
            $allow = (bool) $cliente->get('field_enviar_notificaciones')->value;
            if (!$allow) return false;
        }

        // Email del cliente (AJUSTA el nombre exacto si lo tienes fijo)
        $email = null;
        if ($cliente && $cliente->hasField('field_email_de_contacto') && !$cliente->get('field_email_de_contacto')->isEmpty()) {
            $email = $cliente->get('field_email_de_contacto')->value;
        }

        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            \Drupal::logger('lubriplanner_servicios_api_v2')->warning(
                'Notificar manual: servicio @id sin email válido (cliente: @c).',
    ...
    private function sendProximoServicioEmail(Node $serv): array
    {
        $cliente_id = $this->getTargetId($serv, 'field_cliente_serv');
        $cliente = $cliente_id ? Node::load($cliente_id) : null;
